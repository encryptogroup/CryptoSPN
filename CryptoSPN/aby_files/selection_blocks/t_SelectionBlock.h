/**
 ...
 \author 	masood.naderpour@gmail.com (Original in Java, Thomas Schneider)
...
class Truncated_PN{
	...
	public:
		...
		std::vector<uint32_t> m_vYGates;
		std::vector<bool> m_vYgatesProgram;
		TodoList* Todo;
		Truncated_PN* m_PM;
	};
...
public:
	...
	std::vector<uint32_t> PutCondYGate(std::vector<uint32_t>& a, std::vector<uint32_t>& b, uint32_t s) {
        //std::vector<uint32_t > svec(a.size(), s);
        //return m_cBoolCirc->PutVecANDMUXGate(b,a,svec);//PutCondYGate(a, b, s, true);
...
private:
	...
	uint32_t m_nNumIn;
	uint32_t m_nNumOut;
	std::vector<bool> m_vSwitchGateProgram;
	std::vector<uint32_t> m_vSwapGates; //gate addresses of the swapgates
...
//public:
	...
	std::vector<std::vector<uint32_t> > buildSelectionBlockCircuit(std::vector<std::vector<uint32_t> >& input);
	std::vector<uint32_t> PutCondYGate(std::vector<uint32_t>& a, std::vector<uint32_t>& b, uint32_t s) {
        //std::vector<uint32_t > svec(a.size(), s);
        //return m_cBoolCirc->PutVecANDMUXGate(b,a,svec);//PutCondYGate(a, b, s, true);
