...
std::vector<std::vector<uint32_t> > Truncated_PN::WaksmanPermutation::generateCircuit(std::vector<std::vector<uint32_t> > inputs, std::vector<std::vector<uint32_t> > outputs) {
	...
	if (m_nNumOutputs == 1) {
		...
		std::vector<std::vector<uint32_t> > outtmp(1);
...
void t_SelectionBlock::SelectionBlockProgram(uint32_t *p){
	...
	std::vector<uint32_t> upos, dummy;
...
std::vector<std::vector<uint32_t> > t_SelectionBlock::buildSelectionBlockCircuit(std::vector<std::vector<uint32_t> >& inputs){
	uint32_t rep = inputs[0].size();
	std::vector<std::vector<uint32_t> > out_p1,out_g, outputs;
